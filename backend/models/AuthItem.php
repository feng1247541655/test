...
class AuthItem extends BaseActiveRecord
{
    ...
    public static function createCachedPermissionsGroup()
    {
        $auth = \Yii::$app->authManager;
        $newLine = "\r\n";
        ...
        $groups = $auth->getChildren('root_permission');
        
        foreach ($groups as $group)
        {
            $content .= '$cachedPermissionsGroup[\'' . $group->name . '\'] = [' . $newLine;
        ...
        TFileHelper::writeFile([$dataRoot, 'cache', 'cachedPermissionsGroup.php' ], $content);
    }
